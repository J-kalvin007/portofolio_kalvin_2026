...
/** Période du flottement anti-gravité, en secondes. */
const FLOAT_PERIOD = 8;
...
export default function ImageCarousel({ images, title }: ImageCarouselProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Sans cela, les flèches font aussi défiler la modale sous le carrousel :
      // l'image change et la page bouge en même temps.
      e.preventDefault();
  ...
  /** Style commun aux deux flèches de navigation. */
  const arrowClasses = `cursor-pointer hidden md:flex absolute z-20 items-center justify-center w-14 h-14 rounded-full
    bg-base-content/5 dark:bg-black/20 backdrop-blur-xl border border-base-content/20 dark:border-white/20
    text-base-content/70 dark:text-white/70 hover:text-base-content dark:hover:text-white
    hover:bg-base-content/10 dark:hover:bg-white/20 hover:scale-110 motion-reduce:transform-none
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8 w-full relative px-12 md:px-20" role="group" aria-label={galleryLabel}>
      {/* ── Bouton Précédent ── */}
      <button
        onClick={(e) => { e.stopPropagation(); goPrev(); }}
        className={`${arrowClasses} left-0 lg:left-8`}
      ...
      {/* ── Conteneur flottant anti-gravité ── */}
      {/* Le flottement perpétuel — translation verticale et rotation continues —
          est exactement le type de mouvement que `prefers-reduced-motion` vise.
          Il est neutralisé lorsque la préférence est active. */}
      <motion.div
      ...
      >
        {/* ── Halo ── */}
        {/* L'animation `pulse-glow` a été retirée : ses keyframes ne pilotent que
            `box-shadow`, propriété sans effet visible sur un calque déjà flouté
            en `blur-3xl`. Le halo était donc parfaitement statique malgré
            l'animation déclarée. Une pulsation d'opacité, elle, se voit. */}
        <motion.div
          aria-hidden="true"
          className="absolute -inset-4 bg-[var(--primary)]/15 blur-3xl rounded-3xl"
          animate={shouldReduceMotion ? undefined : { opacity: [0.65, 1, 0.65] }}
          transition={{ duration: FLOAT_PERIOD / 2, repeat: Infinity, ease: 'easeInOut' }}
        />
        ...
        >
          {/* Zone de swipe + images */}
          {/* `mode="wait"` a été retiré : il imposait que l'image sortante ait
              entièrement disparu — 0,6 s — avant que la suivante ne commence à
              apparaître. Un cadre vide s'installait donc entre chaque image, et
              le balayage donnait l'impression d'un temps de chargement. Les deux
              images étant en position absolue, elles se fondent maintenant l'une
              dans l'autre. */}
          <AnimatePresence initial={false} custom={direction}>
            ...
            >
              <Image
                src={images[currentIndex]}
                alt={`${title} — ${imageLabel(currentIndex + 1)}`}
                fill
                sizes="(max-width: 768px) 100vw, 800px"
          ...
          {/* Bordure intérieure luminescente */}
          <div aria-hidden="true" className="absolute inset-0 rounded-[inherit] border-2 border-base-content/10 dark:border-white/20 pointer-events-none" />
        </div>
      </motion.div>
      ...
      <button
        onClick={(e) => { e.stopPropagation(); goNext(); }}
        className={`${arrowClasses} right-0 lg:right-8`}
      ...
      {/* ── Indicateurs de pagination ── */}
      {/* `role="tablist"` a été abandonné : le motif ARIA « onglets » impose des
          `tabpanel` associés, absents ici. Un groupe de boutons décrit par son
          libellé est à la fois plus simple et plus juste. */}
      <div className="flex items-center gap-2" role="group" aria-label={galleryLabel}>
        {images.map((_, i) => (
          <button
            key={i}
            onClick={() => { setDirection(i > currentIndex ? 1 : -1); setCurrentIndex(i); }}
            className={`cursor-pointer rounded-full transition-all duration-300
              focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-base-100 ${i === currentIndex
                ? 'w-6 h-2 bg-[var(--primary)]'
                : 'w-2 h-2 bg-base-content/20 hover:bg-base-content/40 dark:bg-white/30 dark:hover:bg-white/50'
              }`}
